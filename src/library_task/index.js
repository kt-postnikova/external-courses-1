import { bookTemplate, booksContainer, addBookBtn, addBookPopup, addBookForm, searchInput, searchButton, tagPopular, tagAllBooks, addBookPopupClose } from './constants.js'
...
function searchBooks() {
  ...
  const api = getBooks();
  api.then(books => {
    const foundedBooks = books.filter(book => book.title.toLowerCase().includes(searchInput.value.toLowerCase()) || book.author.toLowerCase().includes(searchInput.value.toLowerCase()))
...
function openPopup(popup) {
  ...
  popup.style.transition = 'visibility 0.2s, opacity 0.5s linear';
};
